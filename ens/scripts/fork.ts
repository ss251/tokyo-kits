...
export async function startFork() {
  ...
  const upstream = Bun.env.ENS_RPC_URL || config.defaultRpc
  const source = createPublicClient({ transport: http(upstream, { timeout: 30_000, retryCount: 1, fetchOptions: { headers: { 'User-Agent': 'tokyo-kits/0.1' } } }) })
  assert.equal(await source.getChainId(), 11155111, 'Official ENSv2 preview contracts are on Sepolia 11155111')
  ...
  const child = Bun.spawn(['anvil', '--host', '127.0.0.1', '--port', String(port), '--accounts', '0', '--chain-id', '11155111', '--fork-url', upstream, '--fork-header', 'User-Agent: tokyo-kits/0.1', '--threads', '1', '--fork-block-number', String(blockNumber), '--silent'], { stdout: 'ignore', stderr: 'ignore' })
  const chain = defineChain({ id: 11155111, name: 'Sepolia local fork', nativeCurrency: { name: 'Fork gas', symbol: 'ETH', decimals: 18 }, rpcUrls: { default: { http: [rpcUrl] } } })
  const client = createPublicClient({ chain, ccipRead: false, transport: http(rpcUrl, { timeout: 30_000, retryCount: 1 }), pollingInterval: 50 })
  ...
  try {
    let ready = false
    for (let i = 0; i < 120; i++) {
      if (child.exitCode !== null) throw new Error('Anvil exited before startup; verify Sepolia archive RPC access')
      try { ready = await client.getChainId() === 11155111 } catch { /* startup */ }
      if (ready) break
      await Bun.sleep(250)
    }
    assert(ready, 'Sepolia fork did not start')
    ...
    const official = config.contracts
    ...
    const paths = new Set(['package.json', 'bun.lock', 'addresses.json'])
    for (const dir of ['lib', 'scripts', 'app', 'abi']) {
      for await (const file of new Bun.Glob(`${dir}/**/*`).scan({ cwd: kitRoot, onlyFiles: true })) {
        if (/\.(ts|tsx|css|json|py)$/.test(file)) paths.add(file)
      }
    }
    ...
    async function save(scenario: string, evidence: Record<string, unknown>) {
      assert(['new-app-ensv2', 'add-to-existing'].includes(scenario))
      ...
      const record = { schemaVersion: 1, kind: 'local-fork', scenario, provenAt: new Date().toISOString(), sourceChainId: 11155111, forkChainId: 11155111, forkBlock: blockNumber, forkBlockHash: block.hash, sourceCommit: commit.trim(), sourceDirty: Boolean(status.trim()), sourceFilesSha256, official, codeHashes, runtime: { bun: Bun.version, viem: '2.56.5', contractsSource: config.sourceCommit }, ...evidence }
      await mkdir(resolve(kitRoot, scenario, 'receipts'), { recursive: true })
      const path = resolve(kitRoot, scenario, 'receipts', 'sepolia-latest.json')
