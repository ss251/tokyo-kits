...
| Solidity SwapVM source used by the SDK's integration fixture | `be0a9cc840bb63b9f1abf864a39f4393e1a7312e` | [SDK dependency manifest](https://github.com/1inch/sdks/blob/635d0f10d0c8a2b7df5ca51538ece85a4950942d/package.json), [source](https://github.com/1inch/swap-vm/tree/be0a9cc840bb63b9f1abf864a39f4393e1a7312e) |
...
The SDK packages above were npm `latest` on the check date. The Solidity revisions are the SDK's explicit integration pins, not claims that those commits are repository HEAD or byte-for-byte deployment sources. Current SwapVM HEAD has reorganized `src/` into `contracts/` and changed instruction internals. Import the pinned interfaces; execute against the official deployment.
...
- Use `AquaProgramBuilder` for official-router strategies. Its instruction table is distinct from generic SwapVM. The table's comments are one-based labels, while the encoded byte is the zero-based array index: Extruction is byte `0x20` in the SDK's Aqua table. The custom router example appends its own opcode at byte `0x21` and must be shipped to that custom app address while keeping the official registry.
