...
export async function startFork() {
  ...
  try {
    ...
    async function save(scenario: string, evidence: Record<string, unknown>) {
      ...
      const record = { schemaVersion: 1, kind: 'local-fork', scenario, provenAt: new Date().toISOString(), sourceCommit: commit.trim(), sourceDirty: status.trim().length > 0, compiler: 'solc 0.8.26; optimizer 200; viaIR; cancun', solidityDependencies: dependencies, sourceChain: name, sourceChainId: config.chainId, forkChainId: 8453, forkBlock: blockNumber, forkBlockHash: localBlock.hash, official, codeHashes, fixtureFunding: funding, setupTransactions, ...evidence }
