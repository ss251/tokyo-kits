...
export async function startFork() {
  ...
  const upstream = Bun.env.BASE_RPC_URL || config.defaultRpc
  ...
  const child = Bun.spawn(['anvil', '--host', '127.0.0.1', '--port', String(port), '--accounts', '0', '--chain-id', '8453', '--fork-url', upstream, '--fork-block-number', String(blockNumber), '--silent'], { stdout: 'ignore', stderr: 'ignore' })
  const chain = defineChain({ id: 8453, name: `${name} local fork`, nativeCurrency: { name: 'Fork gas', symbol: 'ETH', decimals: 18 }, rpcUrls: { default: { http: [rpcUrl] } } })
  ...
  try {
    ...
    for (let attempt = 0; attempt < 120; attempt++) {
      ...
      try { ready = await client.getChainId() === 8453 } catch { /* startup */ }
      if (ready) break
      await Bun.sleep(250)
    }
    ...
    const official = { poolManager: config.poolManager as Address, positionManager: config.positionManager as Address, stateView: config.stateView as Address, v4Quoter: config.v4Quoter as Address, universalRouter: config.universalRouter as Address, permit2: config.permit2 as Address, v3SwapRouter: config.v3SwapRouter as Address, v3Quoter: config.v3Quoter as Address, ccaFactory: config.ccaFactory as Address }
    ...
    const manager = await client.readContract({ address: official.positionManager, abi: parseAbi(['function poolManager() view returns (address)']), functionName: 'poolManager' })
    assert.equal(manager.toLowerCase(), official.poolManager.toLowerCase(), 'PositionManager points to a different PoolManager')
    ...
    async function save(scenario: string, evidence: Record<string, unknown>) {
      ...
      const record = { schemaVersion: 1, kind: 'local-fork', scenario, provenAt: new Date().toISOString(), sourceCommit: commit.trim(), sourceDirty: status.trim().length > 0, compiler: 'solc 0.8.26; optimizer 200; viaIR; cancun', solidityDependencies: dependencies, sourceFilesSha256: sourceFiles, sourceChain: name, sourceChainId: config.chainId, forkChainId: 8453, forkBlock: blockNumber, forkBlockHash: localBlock.hash, official, codeHashes, fixtureFunding: funding, setupTransactions, ...evidence }
