...
**Last proven: 2026-09-17 05:54 JST (2026-09-16T20:54:25Z)** — [receipt](receipts/base-latest.json). Authenticated `/lp/create`, `/lp/increase`, and `/lp/decrease` payloads were executed on a Base fork pinned at upstream block 51401344 through the official V3 NonfungiblePositionManager: create `0xd37ad64e040bb0802489d0fbfbd215c6a60af959b71795ff6e3a10f66bf37198` (new full-range WETH/USDC 0.05% position), increase `0xfcd7d3296a9e09c4819c0d2b07eb5eb7f8baff8916e54dfa38f204b4952eb525` and decrease `0x6c31b81ec794a28b780b63faefa958a9eefd224a026d3b4bb5a0c47f08e26807` on the pre-existing public 0.3% position `6013357`, whose owner was impersonated only on the fork. 19 fork transactions in total, including allowance setup and teardown.
...
The shared runner serializes work and runs on a managed Base fork. Missing credentials fail nonzero; that failure was reproduced before compilation or any API request on 2026-09-14 JST, and the same command completed end to end on 2026-09-17 JST once a Developer Platform key was present. The executor picks a recent public WETH/USDC V3 position whose range holds the fork's current tick with at least 100 ticks of margin, because the API quotes against the public chain a few blocks after the fork is pinned and a position at its range edge can otherwise leave range and revert. This executor supports ERC20-only V3 calls and requests `withdrawAsWeth: true`; unsupported selectors or V4 LP execution fail explicitly. An unsigned API transaction has status `UNSIGNED_NOT_EXECUTED` and is never treated as a proof receipt.
...
The client validates sender, chain, nonempty calldata, native value, request ID, and response token ordering. A full-range create legitimately returns `adjustedMinPrice` `"0"`; the client accepts a zero minimum only in responses and still requires positive request bounds. The shared executor validates official NFPM calldata recursively, permits only the requested mutation/collection, binds the NFT/pair/range/recipient, enforces quote and request limits, and grants then revokes exact input allowances. The live API applies `slippageTolerance` to the pool price exactly as the official V3 SDK does, not to token amounts, so the executor reads the fork's pool state before each action and recomputes the acceptable minimums with the pinned `@uniswap/v3-sdk` (allowing 5 bps of quote-to-fork drift). It checks actual token deltas and minted NFT metadata as well as liquidity changes. Nine regression tests, one built from a live position observed on 2026-09-17, reject excess spending, unrelated calls, and weaker slippage protection. API `simulateTransaction: false` permits local fork preparation; it does not constitute execution proof.

The receipt above records, for each endpoint, the date, fork block, official destination, API request ID, successful transaction receipt, NFT ID, and before/after ownership or liquidity, keeping the fork-created and pre-existing public NFT IDs distinct. It stores no API key or private signing material; public transaction signatures in executed calldata are part of transaction evidence. Rerunning `make demo` selects the newest in-range public position again and refreshes the receipt.
